...
const petalRotationStickToParent = ["Missile", "Dandelion", "Web", "Fire Missile", "Carrot"] //These petals are rotated to match to their rotation around parent flower
...
const petalRenderMap = {
    ...
    */
    ...
    Root: (p) => {
        ...
        ctx.lineWidth = p.radius / divCoef / 2.2;
        ctx.fillStyle = blendColor('#8b5529', '#FF0000', blendAmount(p)); // Dark brown
        ctx.strokeStyle = blendColor('#5c3617', '#FF0000', blendAmount(p)); // Even darker outline
        if (checkForFirstFrame(p)) {
            ctx.fillStyle = "#FFFFFF";
            ctx.strokeStyle = "#FFFFFF";
        }
        ...
        ctx.beginPath();
        ctx.moveTo(p.radius * -0.3, p.radius * 0.8);
        ctx.lineTo(p.radius * -0.1, p.radius * 0.1);
        ctx.lineTo(p.radius * -0.25, p.radius * -0.4);
        ctx.lineTo(p.radius * 0.0, p.radius * -0.7);
        ctx.lineTo(p.radius * 0.1, p.radius * -0.6);
        ctx.lineTo(p.radius * 0.05, p.radius * -0.4);
        ctx.lineTo(p.radius * 0.3, p.radius * 0.4);
        ctx.lineTo(p.radius * 0.1, p.radius * 0.6);
        ctx.closePath();
        ctx.fill();
        ctx.stroke();

        // Inner shape (light brown)
        ctx.fillStyle = blendColor('#b26a34', '#FF0000', blendAmount(p)); // Lighter brown
        ...
        ctx.beginPath();
        ctx.moveTo(p.radius * -0.2, p.radius * 0.6);
        ctx.lineTo(p.radius * 0.05, p.radius * -0.3);
        ctx.lineTo(p.radius * 0.2, p.radius * 0.3);
        ctx.lineTo(p.radius * 0.05, p.radius * 0.5);
        ctx.closePath();
        ctx.fill();
    },
    Stick: (p) => {
    ...
    Carrot: (p) => {
        ...
        ctx.rotate(-Math.PI/2)
        ctx.beginPath();
        ctx.moveTo(-0.2 * p.radius, -1.12 * p.radius)
        ctx.quadraticCurveTo(0.08 * p.radius, -1.17 * p.radius, 0.31 * p.radius, -1.06 * p.radius)
        ctx.quadraticCurveTo(0.3 * p.radius, -1.31 * p.radius, 0.78 * p.radius, -1.44 * p.radius)
        ctx.quadraticCurveTo(0.75 * p.radius, -1.67 * p.radius, 0.27 * p.radius, -1.53 * p.radius)
        ctx.quadraticCurveTo(0.12 * p.radius, -2.32 * p.radius, -0.16 * p.radius, -2.23 * p.radius)
        ctx.quadraticCurveTo(-0.05 * p.radius, -1.73 * p.radius, -0.15 * p.radius, -1.54 * p.radius)
        ctx.quadraticCurveTo(-0.22 * p.radius, -1.68 * p.radius, -0.73 * p.radius, -1.76 * p.radius)
        ctx.quadraticCurveTo(-0.21 * p.radius, -1.29 * p.radius, -0.2 * p.radius, -1.12 * p.radius)
        ctx.fill();
        ctx.stroke();
        ctx.closePath();
        ...
        ctx.bezierCurveTo(0.63 * p.radius, -0.96 * p.radius, 0.76 * p.radius, 0.25 * p.radius, 0, p.radius * 1.65)
        ctx.moveTo(0, -p.radius)
        ctx.bezierCurveTo(-0.63 * p.radius, -0.96 * p.radius, -0.76 * p.radius, 0.25 * p.radius, 0, p.radius * 1.65)
        ctx.fill();
        ctx.stroke();
