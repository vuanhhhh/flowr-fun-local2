...
class Enemy {
	constructor(init) {
		...
		if (['Tarantula', 'Spider', 'Worker Ant', 'Soldier Ant', "Soldier Fire Ant", 'Beetle', 'Scorpion', 'Locust', 'Queen Ant', 'Queen Shiny Ant', 'Queen Fire Ant', 'Desert Moth', 'Starfish', 'Crab', 'Baby Ant', 'Worker Fire Ant', 'Baby Fire Ant', "Soldier Shiny Ant"].includes(this.type)) {
...
let normalEnemyRenderMap = {
	...
	"Tanksmith": (e) => {
		ctx.fillStyle = blendColor("#790000", "#FF0000", Math.max(0, blendAmount(e)));
		ctx.strokeStyle = blendColor("#550000", "#FF0000", Math.max(0, blendAmount(e)));
		if (checkForFirstFrame(e)) {
			ctx.fillStyle = "#FFFFFF";
			ctx.strokeStyle = "#FFFFFF";
		}
	...
	Whirlpool: (e) => {
            ...
            ctx.beginPath();
            for (let i = 10; i--; i > 0) {
                ctx.lineTo(e.render.radius * Math.cos(i * Math.PI / 4.5) * 2 / 3.25, e.render.radius * Math.sin(i * Math.PI / 4.5) * 2 / 3);
            }
            ctx.fill();
            ctx.stroke();
            ctx.closePath();
            ...
            ctx.beginPath();
            for (let i = 10; i--; i > 0) {
                ctx.lineTo(e.render.radius * Math.cos(i * Math.PI / 4.5) * 1 / 3.25, e.render.radius * Math.sin(i * Math.PI / 4.5) * 1 / 3.5);
            }
            ctx.fill();
            ctx.stroke();
            ctx.closePath();

            ctx.rotate(-e.renderRotation * 3.3);
	},
	Tree: (e) => {
		// Outer 7-point star (dark green)
		ctx.lineWidth = 0;
		ctx.fillStyle = blendColor('#395c2f', '#FF0000', blendAmount(e));
		if (checkForFirstFrame(e)) ctx.fillStyle = "#FFFFFF";

		ctx.beginPath();
		for (let i = 0; i < 7; i++) {
				const outerAngle = (Math.PI * 2 * i) / 7;
				const innerAngle = outerAngle + Math.PI / 7;
				const outerX = Math.cos(outerAngle) * e.radius * 1.15;
				const outerY = Math.sin(outerAngle) * e.radius * 1.15;
				const innerX = Math.cos(innerAngle) * e.radius * 0.8;
				const innerY = Math.sin(innerAngle) * e.radius * 0.8;
		...
		// Inner 7-point star (lighter green)
		ctx.fillStyle = blendColor('#4c7a3e', '#FF0000', blendAmount(e));
		if (checkForFirstFrame(e)) ctx.fillStyle = "#FFFFFF";
		...
		for (let i = 0; i < 7; i++) {
				const outerAngle = (Math.PI * 2 * i) / 7;
				const innerAngle = outerAngle + Math.PI / 7;
				const outerX = Math.cos(outerAngle) * e.radius * 0.7;
				const outerY = Math.sin(outerAngle) * e.radius * 0.7;
				const innerX = Math.cos(innerAngle) * e.radius * 0.42;
				const innerY = Math.sin(innerAngle) * e.radius * 0.42;
